...
$\Delta N_{\rm eff}$ safe & \checkmark & \checkmark & \checkmark & $\sim$ & $\sim$ & \checkmark \\
Dark energy              & separate   & separate   & separate & separate & separate & \checkmark \\
$H_0$ prediction         & input      & input      & input  & input  & input  & \textbf{67.4} \\
...
\section{The $A_4$ Symmetry and the Neutrino Question}
\label{disc:neutrino}
...
relic angle $\theta_{relic} = 19.47^\circ$ — testable with DUNE and Hyper-K.
...
\begin{enumerate}
\item \textbf{Mediator overclosure (FATAL).}
      The $\phi$ relic abundance must be depleted by cannibal processes
      $3\phi \leftrightarrow 2\phi$. A full thermal history calculation
      (beyond the simple estimate $\Omega_\phi/\Omega_\chi \sim 5\times10^{-4}$)
      has not been performed. If the cannibal cross section is too small,
      $\phi$ could overclose the universe. This requires a dedicated
      Boltzmann solver tracking both $\chi$ and $\phi$ simultaneously.
...
\item \textbf{Mixed Majorana coupling.}
      ...
      In reality, the relic angle $\theta_{relic} = 19.47^\circ$ means the
...
\item \textbf{DESI DR2 equation of state.}
      The model predicts $w_0 \approx -0.727$, while DESI DR2 data
      (2025) favor $w_0 = -0.827 \pm 0.063$, i.e., a $\sim 1.6\sigma$ tension.
      A steeper thawing potential (or a more complex dark axion potential beyond
      the cosine approximation) may be needed if this tension persists. This is
      a \emph{testable prediction}: future DESI data releases will either confirm
      or rule out the predicted $w_0$.
...
\item The same Lagrangian, evaluated as a path integral, proves that the VPM
      ...
      confinement predicts $H_0 = 67.4$~km/s/Mpc (Chapter~2).
...
\item The CP phase, promoted to a dynamical dark axion $\sigma$, provides an
      ...
      the observed vacuum energy (Chapter~3).
\end{enumerate}
...
literature comparison, and blind stress tests.
...
($t_{\rm gc}/t_{\rm univ} > 10^4$), $w_0 \approx -0.73$ (testable with DESI),
and a universal relic angle $\theta_{\rm relic} = 19.47^\circ$ independent of
$(m_\chi, m_\phi, \alpha)$.
