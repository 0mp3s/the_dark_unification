...
For our best-fit parameters, the relic angle was computed numerically:
\begin{equation}
  \theta_\text{relic} = 19.47° = \arcsin\!\left(\tfrac{1}{3}\right) = \arccos\!\left(\tfrac{2\sqrt{2}}{3}\right)
\end{equation}

This is the \textbf{tetrahedral angle} --- the angle between a vertex of a regular tetrahedron and its centroid. It is also the value of the Cabibbo angle in certain A$_4$-symmetric flavour models.
...
Its Clebsch-Gordan coefficients give:
$$\frac{g_p^2}{g_s^2} = \frac{1}{9}, \qquad 
  \sin^2\theta = \frac{1}{9} \times \frac{v_p^2}{v_s^2}$$
For $v_p/v_s = 1.061$ (derived from flavon potential minimisation), this gives 
$\sin^2\theta = 1/8$, corresponding to $\theta = 19.47°$ exactly.\\[0.3em]
\textbf{The tetrahedral angle is not a coincidence --- it is a prediction of A$_4$ symmetry.}
\end{insightbox}
...
\paragraph{Chapter 3 (T-breaking):} Derives the dark energy equation of state from the misalignment mechanism, shows the tetrahedral angle $\theta = 19.47°$ arises from A$_4$, and presents the DESI DR2 comparison.
...
\item \textbf{The $\sigma$ Dynamics Problem.}\\
  After relic production, the $\sigma$ field oscillates and thermalises with the dark sector. Whether it settles at $\theta_\text{relic} = 19.47°$ (tetrahedral) or rolls to $\theta = \pi/2$ (CW minimum) depends on the competition between the alignment potential and the CW potential. This competition is not yet analytically solved.
...
\begin{tabular}{lll}
  ...
  \hspace{1em}$\theta_\text{relic}$ & $19.47°$ = $\arcsin(1/3)$ & CW + A$_4$ \\
