...
The projection onto the real axis ($y\cos\theta_{\rm relic} = 0.949\,y$) governs
SIDM; the projection onto the imaginary axis ($y\sin\theta_{\rm relic} = 0.316\,y$)
governs dark energy. The ratio of dark energy to dark matter coupling strengths
is $\tan^2\theta_{\rm relic} = 1/9$ --- explaining why $\Omega_{DE}/\Omega_{DM}$
...
$\Delta N_{\rm eff}$ & $0.16$--$0.26$\footnote{This arises from dark gluons
  (SU(2)$_d$, 6 d.o.f.)\ that remain relativistic at BBN since
  $\Lambda_d \sim 2$~meV $\ll T_d$.  The massive species
  ($\chi$, $\phi$) are Boltzmann-suppressed and contribute negligibly.}
  & $< 0.34$ (Planck+BAO) & CMB-S4 \\
\bottomrule
\end{tabular}
\end{table}
