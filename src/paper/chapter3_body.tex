...
The analogy is not merely verbal --- it rests on a precise mathematical
correspondence. In electromagnetism, the field strength tensor $F_{\mu\nu}$
...
\begin{enumerate}
  ...
  \item $y_p$ generates a vacuum energy through the Coleman-Weinberg
        ...
        depends on $M_{\rm eff}(\theta) = m_\chi\sqrt{\cos^2\theta + \sin^2\theta/9}$,
...
specific point on this internal circle ($\theta_{\rm relic} = 19.47^\circ$),
...
\begin{tikzpicture}[scale=2.8]
  ...
  % angle
  \draw[red,very thick] (0,0) -- ({cos(19.47)},{sin(19.47)});
  \fill[red] ({cos(19.47)},{sin(19.47)}) circle (1.5pt)
    node[above right,font=\small] {$\theta_{\rm relic} = 19.47^\circ$};
  % angle arc
  \draw[red,thick,->] (0.4,0) arc (0:19.47:0.4);
  \node[red,font=\footnotesize] at (0.5,0.1) {$\theta$};
  % projections
  \draw[dashed,gray] ({cos(19.47)},{sin(19.47)}) -- ({cos(19.47)},0)
    node[below,font=\footnotesize,black] {$y\cos\theta$};
  \draw[dashed,gray] ({cos(19.47)},{sin(19.47)}) -- (0,{sin(19.47)})
...
geometry --- the relic angle sits at $19.47^\circ$, not $0.001^\circ$ or
$89.999^\circ$, because both SIDM and relic density demand it.}
...
$\theta_{\rm relic} = 19.47^\circ$. We show this does not happen.
...
\midrule
$\theta_{\rm relic}$ & $19.47^\circ$ & --- (no direct measurement) & Future \\
...
Majorana fingerprint & $R > 1$ at $v \sim 53$ km/s & Not yet measured & Future \\
SMBH seeds   & $t_{\rm gc}/t_{\rm univ} > 10^4$ & No observed seeds & \checkmark \\
Baryonic 5th force & 0 & No detection & \checkmark \\
$\Delta N_{\rm eff}$ & $< 10^{-10}$ & $< 0.30$ (Planck) & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
